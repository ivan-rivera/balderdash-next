...
import {baseSessionValidators, joinSessionValidators, newSessionValidators} from "../../lib/validators";
...
function handleSubmission(request, username, setErrorVisible, setErrorMessage) {
    request
        .then(result => {
            ...
            if (!error) {
                console.log('Session submission handled successfully');
                cookieCutter.set('username', username);
                Router.push('/[sessionId]', `/${sessionId}`)
            } else displayError(error, setErrorVisible, setErrorMessage)
        })
...
export default function SessionModal(props) {
    ...
    const scenarioValidators = props.join ? joinSessionValidators : newSessionValidators
    const form = useForm({
        initialValues: {username: '', sessionId: ''},
        validate: {...baseSessionValidators, ...scenarioValidators}
    })
    return (
        <Modal opened={props.opened} onClose={() => props.setOpened(false)} title={props.title}>
            <form onSubmit={form.onSubmit(v => handleSubmission(
                    props.join ? joinSession(v.sessionId, v.username) : initSession(v.username, v.rounds),
                    v.username, setErrorVisible, setErrorMessage
                ))}>
                <UsernameInput form={form}/>
                { props.join && <SessionIdInput form={form}/> }
                { !props.join && <RoundsInput form={form}/> }
                <Button type="submit" color="red" mt="xl" mb="xl" style={{width: '100%'}}>Go!</Button>
            </form>
